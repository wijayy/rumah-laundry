<div>
    <form wire:submit='save' class="space-y-4">
        <flux:input wire:model='nama' :label="'Nama Customer'" required></flux:input>
        <flux:input wire:model='selesai' min="{{ date('Y-m-d') }}" type="date" :label="'Estimasi Selesai'" required></flux:input>
        <flux:select wire:model='payment' :label="'Pembayaran'" required>
            ...
            @foreach ($services as $index => $item)
                <div class="flex gap-4 items-end">
                    <div class="flex gap-4 w-10/12">
                        <flux:select wire:model='services.{{ $index }}.id' wire:change='updateServices' :label="'Service'">
                        ...
                        @if ($item['satuan'] == 'kg')
                            <flux:input type="number" required wire:input='updateTotal' class="" min="0" step="0.1"
                                wire:model='services.{{ $index }}.jumlah' :label="'Jumlah'" kbd="{{ $item['satuan'] }}">
                            </flux:input>
                        @else
                            <flux:input type="number" required wire:input='updateTotal' class="" min="0" wire:model='services.{{ $index }}.jumlah'
                                :label="'Jumlah'" kbd="{{ $item['satuan'] }}"></flux:input>
                        @endif
                        {{-- <flux:input type="text" class="w-fit!" wire:model='services.{{ $index }}.satuan'
                            :label="'Satuan'">
                        </flux:input> --}}
