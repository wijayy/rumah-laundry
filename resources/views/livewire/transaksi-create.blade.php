<div>
    <form wire:submit='save' class="space-y-4">
        ...
        <flux:input wire:model.live='selesai' min="{{ date('Y-m-d') }}" type="date" :label="'Estimasi Selesai'"
            required></flux:input>
            ...
            @foreach ($services as $index => $item)
                <div class="flex gap-4 items-end">
                    <div class="flex gap-4 w-10/12">
                        <div class="w-2/3">
                            <flux:select wire:model.live='services.{{ $index }}.id' wire:change='updateServices'
                                :label="'Service'">
                                @foreach ($service as $itm)
                                    <flux:select.option class="text-xs!" value="{{ $itm->id }}">{{ $itm->nama }}
                                    </flux:select.option>
                                @endforeach
                            </flux:select>
                        </div>
                        <div class="w-1/3">
                            @if ($item['satuan'] == 'kg')
                                <flux:input type="number" required wire:input='updateTotal' class="" min="0" step="0.1"
                                    wire:model.live='services.{{ $index }}.jumlah' :label="'Jumlah'"
                                    kbd="{{ $item['satuan'] }}">
                                </flux:input>
                            @else
                                <flux:input type="number" required wire:input='updateTotal' class="" min="0"
                                    wire:model.live='services.{{ $index }}.jumlah' :label="'Jumlah'"
                                    kbd="{{ $item['satuan'] }}"></flux:input>
                            @endif
                        </div>
